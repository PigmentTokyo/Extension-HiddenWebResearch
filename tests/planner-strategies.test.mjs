...
assert.match(getResearchPlannerInstruction('claude'), /profile recommendation is one highest-value precise first query/u);
assert.match(getResearchPlannerInstruction('claude'), /never claim this is Anthropic private wording/u);
assert.match(getResearchPlannerInstruction('claude'), /recommendation for later rounds is one sequential follow-up/u);
